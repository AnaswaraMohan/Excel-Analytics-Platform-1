...
class AnalyticsService {
  ...
  analyzeTrends(data) {
    ...
    // Simple trend analysis for numeric columns over sequence
    numericColumns.forEach(column => {
      const values = data
  ...
  calculateLinearTrend(points) {
    ...
    const sumY2 = points.reduce((sum, point) => sum + (point.value * point.value), 0);
    ...
    // Calculate R-squared
    const meanY = sumY / n;
    const totalSumSquares = points.reduce((sum, point) => sum + Math.pow(point.value - meanY, 2), 0);
  ...
  calculateValidityScore(data) {
    // Simplified validity score based on data consistency
    let validityScore = 100;
    
    // Deduct points for obvious data issues
    const columns = Object.keys(data[0] || {});
    columns.forEach(column => {
      ...
      if (uniqueTypes.length > 2) {
        validityScore -= 10; // Multiple types in same column
      }
    });
  ...
  generateDataQualityRecommendations(completeness, uniqueness, consistency, validity) {
    ...
    if (completeness < 90) {
      recommendations.push('Consider data imputation or collection improvement for missing values');
    }
    if (uniqueness < 95) {
      recommendations.push('Review and remove duplicate records to improve data quality');
    }
    if (consistency < 85) {
      recommendations.push('Standardize data types and formats across columns');
    }
    if (validity < 90) {
      recommendations.push('Implement data validation rules to ensure data integrity');
    }

    return recommendations;
  }

  generateTrendPrediction(trend, currentLength) {
    const nextValue = trend.slope * (currentLength + 1) + trend.intercept;
    return {
      nextPredictedValue: this.roundToDecimal(nextValue, 4),
  ...
  async generateInsights(analysisResults, metadata = {}) {
    try {
      ...
      return {
        insights: [{
          category: 'general',
          finding: 'Analysis completed successfully. AI insights temporarily unavailable.',
          confidence: 0.8,
          priority: 'medium',
          impact: 'moderate'
        }],
        fullResponse: 'Data analysis completed with statistical findings.',
