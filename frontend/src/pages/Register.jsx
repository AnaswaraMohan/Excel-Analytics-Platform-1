...
const Register = ({ isModal = false, onSwitchModal }) => {
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div className="relative">
          <input
            ...
            onChange={onChange}
            required
            className={`w-full px-4 py-3 rounded-lg bg-transparent border ${isModal ? 'border-white/20' : 'border-jet-300'} ${password && confirmPassword && password !== confirmPassword ? 'border-red-500/50' : ''} focus:border-pigmentgreen-400 text-white focus:ring-0 focus:outline-none transition-colors duration-200 pr-10`}
      ...
      {!isModal && (
        <div className="mt-6 text-center">
          <p className="text-sm text-jet-500">
            Already have an account?{' '}
            <Link to="/login" className="font-medium text-pigmentgreen-600 hover:text-pigmentgreen-500">
              Sign In
            </Link>
          </p>
        </div>
  ...
  return (
    <AuthLayout
      ...
      subtitle="Join millions of users analyzing data with ease"
    >
      {FormContent}
    </AuthLayout>
  );
